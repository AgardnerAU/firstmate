...
state/               runtime records and signals; gitignored
  ...
  .branch-eligible-rows .main-eligible-rows  queue-lock-serialized per-actor wake-row claims; the extension grants branch rows through bin/fm-wake-grant.sh before prompting, and bin/fm-wake-drain.sh presents and acknowledges only the current actor's opaque row set (docs/watcher-continuity.md "Per-actor acknowledgement")
