...
state/               runtime records and signals; gitignored
  ...
  .branch-eligible-rows .branch-eligible-owner .main-eligible-rows  queue-lock-serialized per-actor wake-row claims plus the branch claim's live process/generation evidence; the extension grants branch rows through bin/fm-wake-grant.sh before prompting, and bin/fm-wake-drain.sh presents and acknowledges only the current actor's opaque row set (docs/watcher-continuity.md "Per-actor acknowledgement")
