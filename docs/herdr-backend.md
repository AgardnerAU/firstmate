...
That boundary is owned once, in `fm_backend_herdr_agent_status_raw` - the single `agent get` reader every native-state consumer in this adapter funnels through (`fm_backend_herdr_busy_state`, the submit baseline, `fm_backend_herdr_wait_for_working`, the queued-Enter busy primitive, and the event-stream reconcile) - rather than being re-checked at each call site.
