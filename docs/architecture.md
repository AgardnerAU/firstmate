...
The ledger has no run ID, so equal heads do not identify reruns; a terminal row supplies newer failed-state truth when its observable status, head, or pull request differs from the full run answer, or when the reader reached it through the coarse fallback.
When all observable terminal fields agree and there is no later ordering evidence, the failure stands so a real failure is never hidden, while a stable status-line snapshot prevents a concurrent append from lending new ordering evidence to an older line and its mtime must clear the ledger start's entire minute.
Independently of outcome, a terminal reading publishes a pull request only while the newest current-run evidence still attributes that reading, so a stale success cannot lend its pull request to an active rerun.
