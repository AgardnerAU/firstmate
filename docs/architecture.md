...
A later run the ledger proves current answers instead, a later self-declared pause or completion the ledger's date proves post-dates the failed run answers instead, and a later run that cannot be bound here reads as unknown; with no ordering evidence the failure stands, so a real failure is never hidden.
