...
A word the reader has already proven older than the run it reported on contradicts nothing, so the routine shape of a `done:` written before a run that then fails still reaches the captain; only a word whose ordering against the run was never established keeps the suppression.
