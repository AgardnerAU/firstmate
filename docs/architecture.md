...
The stable safety boundary is that a stale terminal reading - a failure or a success - cannot outrank newer authoritative evidence, uncertain later history reads unknown, and a terminal run publishes no pull request unless current-run evidence still attributes it.
