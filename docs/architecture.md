...
An exact `worker-state` record written by `fm-control stand-down` excludes a dead endpoint from the watcher's per-window supervision - pane-stale polling, wedge escalation, and the steering-inbox re-ring ladder alike - while the held task has intentionally no worker, but a live endpoint at that record immediately returns to ordinary stale and wedge detection. Nothing is left unread by that: `stand-down` refuses while an unacknowledged instruction is still waiting, and `fm-send` refuses to add one afterwards.
