...
When all observable terminal fields agree and there is no later ordering evidence, the failure stands so a real failure is never hidden, while a stable status-line snapshot prevents a concurrent append from lending new ordering evidence to an older line.
