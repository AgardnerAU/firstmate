...
Both kinds are checked for an in-flight run the same way; the only difference is that a scout's ordinary detached-HEAD scratch worktree has no branch for a run to own, while a ship at a detached HEAD has lost the attribution a run is keyed by and refuses.
