...
Each registered source has its own child process blocking on that source, and the watcher's per-cycle `reconcile` republishes every captured result with no durable handled acknowledgement yet whose last wake-queue announcement is at least `FM_PROCEVENT_REANNOUNCE_SECONDS` old (default 300, range 0..86400, 0 meaning every reconcile), restarts a source whose owner is gone, and stops this home's runner when reconciliation runs after its registration disappeared unexpectedly.
