...
| a note is not an answer | The same suite answers four held calls from one board round and proves a selected option and the typed words of a freeform-only card close their calls, while a note with no selection on a card with options - marked a comment, or a current-shape row with no intent - leaves its call held and still queues the wake |
| board receipt | The same round stages one receipt naming the recorded calls and the calls still open, and reports a changed answer re-sent to an already-closed call as not applied, never as still open, with a follow-up even when it is the round's only row, and `tests/fm-procevent.test.sh` proves a firstmate-owned Lavish source posts the staged receipt once as the next listener's agent reply, with no handler turn; a live check on 2026-09-24 on macOS (Darwin 27.0.0) with `lavish-axi` 0.1.67 in a scratch home showed the receipt in the board's Conversation panel after the next listener started |
...
| bounded re-announcement until handled | a durably captured result with no handled acknowledgement is re-announced by `reconcile` with the same source and sequence - not only the first restart after a crash - once its last announcement is at least `FM_PROCEVENT_REANNOUNCE_SECONDS` old, never earlier under the default or a nonzero interval, on every call at `0`, and an unusable interval is refused by name; a presented-but-unacknowledged wake resurfaces identically after a simulated replacement session |
| away and quiet escalation of one board result | `tests/fm-daemon.test.sh` drives five watcher cycles of real `reconcile` and the daemon's durable-wake handling over one unhandled Lavish capture and proves it reaches the escalation buffer once; after `handled`, a flush delivers only other items and a buffer holding only the handled result delivers nothing; a buffered board answer flushes without waiting for the batch window while the busy and composer guards still defer it; `tests/fm-afk-return.test.sh` proves the away-return catch-up omits a handled result and keeps an unhandled one |
